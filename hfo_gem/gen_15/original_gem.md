...
**[NOTE: Red/Blue terminology flagged for rename - see Pain #18 above]**
...
- [NEW] Pain #18: AI bypass behavior (immediate reward hacking when safeguards trigger)
- [NEW] Mnemonic issue: Red/Blue too generic, user can't remember (needs vivid replacement)
- [HIGH PRIORITY] Add bypass detection to git hooks (--no-verify, --skip-hooks)
- [MEDIUM PRIORITY] Rename Red/Blue mnemonic (post-L1, affects all docs)
- Current priority: L0→L1 automation (per 80/20 analysis, defer hardening)
...
# Root policy check: Enforce exactly one  folder at root to prevent fragmentation/hallucinations
HIVE_FOLDERS=$(ls -1 | grep '^' | wc -l)
if [ "$HIVE_FOLDERS" -gt 1 ]; then
  echo "Policy violation: Root has multiple  folders (web/mountain emojis); only 1 allowed for singleton HiveFleetObsidian"
  exit 1
fi

# Hive Fleet Obsidian Pre-Commit Hook: Validates active singleton GEM1 file

TARGET_FILE="_hive-fleet-obsidian-october-2025/_gem_gene_seed/Singleton/🧬_Gem1_Pass15_20251023T000000Z.md"

if [ ! -f "$TARGET_FILE" ]; then
  echo "Error: Active singleton file not found: $TARGET_FILE"
  exit 1
fi

LINE_COUNT=$(wc -l < "$TARGET_FILE")
if [ "$LINE_COUNT" -ne 2433 ]; then
  echo "Error: Singleton file has incorrect line count: $LINE_COUNT (expected 2433)"
  exit 1
fi

# Overmind policy: Absolute ban on ⌛ files/paths until explicit approval; ⌛ permitted as text only in active Gem1 Pass15
COMMIT_MSG_FILE=".git/COMMIT_EDITMSG"
OVERRIDE=false
if [ -f "$COMMIT_MSG_FILE" ]; then
  if grep -q "\[OVERMIND_APPROVAL\]" "$COMMIT_MSG_FILE"; then
    OVERRIDE=true
    echo "$(date): Overmind override for ⌛ ban policy in commit $(git rev-parse --short HEAD 2>/dev/null || echo 'pending')" >> override.log
  fi
fi

if [ "$OVERRIDE" = false ]; then
  if git diff --cached --name-only | grep -q -E '^.*⌛|.*⌛|.*⌛'; then
    echo "Overmind policy: No ⌛ files/paths until explicit approval (Swarmlord offline)"
    exit 1
  fi
fi

echo "Pre-commit validation passed: Singleton intact."
```

### Root Singleton Enforcement
Enforce exactly one  root folder to maintain SSOT and prevent fragmentation. Pre-commit hook validates: `ls -1 | grep '^' | wc -l` must equal 1.

### Overmind ⌛ Lockdown
⌛ (hourglass) permitted as text-only in active GEM; no ⌛ creation in files/paths until overmind approval. Pre-commit hook blocks unless overridden with [OVERMIND_APPROVAL] in commit message (logged to override.log). Swarmlord offline; approval pending reactivation.

### Stigmergy Pattern Reference
Key rules embedded from stigmergy-pattern.md:

#### Core Principles
1. **Single  Root Folder**: Exactly one root-level ``-prefixed folder (e.g., `_hive-fleet-obsidian-october-2025`). No duplicates; violations indicate drift.
2. **Sub-Emergent Structure**: All content in subdirectories (e.g., `/immune_system/`, `/_gem_gene_seed/Singleton/`, `/_blackboard/`).
3. **No Duplicates**: 🥇 for active singletons only; remove from archives. Root contains only single `` folder, `.git/`, essentials.

#### Enforcement Mechanisms
- **Pre-Commit Hook**: Validates singleton, strips 🥇 from non-actives.
- **Validation Scripts**: `validate_gem1.sh` confirms SSOT (e.g., 2433 lines in GEM).
- **Git Workflow**: "stigmergy" in commit messages for structural changes.

#### Violation Handling
- Detect via regex for 🥇 outside actives or multiple roots.
- Remediate: Consolidate to `/archives/`, rename, commit.

Active GEM reference: `_hive-fleet-obsidian-october-2025/_gem_gene_seed/Singleton/🧬_Gem1_Pass15_20251023T000000Z.md` (2433 lines).

This pattern self-enforces via hooks, evolves through GEM updates.